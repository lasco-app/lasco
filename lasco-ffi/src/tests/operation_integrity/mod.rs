...
fn one_remote_preserves_operations_across_compaction() {
    ...
    let initial_operation_count = device.library.list_operations(0, u64::MAX).unwrap().len();
    ...
    let operations = device.library.list_operations(0, u64::MAX).unwrap();
    ...
    let fetched_operations = joined_device.library.list_operations(0, u64::MAX).unwrap();
    assert_operations_are_complete_and_unique(
        &fetched_operations,
        initial_operation_count + created_operation_count,
    );
