...
impl FfiLibrary {
    pub async fn lasco_cloud_storage_usage(&self) -> Result<u64, LascoError> {
        let auth = self.inner.cloud_runtime().lasco_cloud_auth().ok_or_else(|| LascoError::Other { msg: "Lasco Cloud is not configured".to_string() })?;
        self.rt.spawn(async move { auth.storage_usage().await }).await
            .map_err(|e| LascoError::Other { msg: e.to_string() })?
            .map(|usage| usage.approximate_used_bytes).map_err(|e| LascoError::Other { msg: e.to_string() })
    }
    ...
    pub async fn lasco_cloud_check_initial_import(&self, media_bytes: u64) -> Result<(), LascoError> {
        let config = self.load_library_json()?;
        let cloud_remotes: Vec<_> = config.remotes.iter().filter_map(|remote| match &remote.kind {
            lasco_core::library_json::RemoteKind::CloudS3(cloud) => {
                self.inner.cloud_runtime().register_remote(remote.remote_uuid, cloud.cloud_storage_id.clone());
                Some(remote.remote_uuid)
            }
            _ => None,
        }).collect();
        if cloud_remotes.is_empty() { return Ok(()); }
        let runtime = self.inner.cloud_runtime();
        let checked = self.rt.spawn(async move { runtime.check_storage_usage_for_remotes(&cloud_remotes, media_bytes).await }).await
            .map_err(|e| LascoError::Other { msg: e.to_string() })?
            .map_err(|e| LascoError::CloudQuotaExceeded { msg: e.to_string() })?;
        if checked.allowed { Ok(()) } else { Err(LascoError::CloudQuotaExceeded { msg: format!("{} bytes requested with {} of {} bytes already indicated", checked.proposed_media_bytes, checked.approximate_used_bytes, checked.storage_quota_bytes) }) }
    }
