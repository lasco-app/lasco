...
final class ManageModel {
    private(set) var defaultUploadAlbumID: String?
    ...
    init(repository: any LibraryRepositoryProtocol, session: LibrarySessionState) {
        ...
        defaultUploadAlbumID = session.defaultUploadAlbumID
        autoImportDeviceMedia = session.autoImportDeviceMedia
    }
    ...
    func start() async {
        ...
        syncFromSession()
        for await change in stream {
            guard change == .all || change == .session else { continue }
            syncFromSession()
        }
    }

    func setDefaultUploadAlbum(id: String?) async throws {
        try await repository.setDefaultUploadAlbum(albumID: id)
    }
