...
into the target's last-known-state cache. `CrdtStateReplica.outgoing` is a durable local-mutation
outbox used for crash recovery: it repairs a log append interrupted after the snapshot write, but
is not Push's source of truth because it excludes operations learned by Fetch.
