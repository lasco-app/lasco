...
2. **Immutability.** Once created, a file is never modified in place. The local `crdt-state.enc` snapshot is the exception: it is atomically replaced as the durable CRDT state and outgoing-operation outbox change.
