...
2. **Immutability.** Once created, a file is never modified in place. The local `crdt-state.enc` snapshot is the exception: it is atomically replaced as the durable CRDT state changes.
...
`CrdtState` is a CRDT rather than a replay log. Each immutable operation has a Lamport dot,
and applying a duplicate is idempotent. LWW registers use dots for deterministic conflict resolution,
